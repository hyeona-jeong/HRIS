...
class Edit(QMainWindow, form_class):
    ...
    def __init__(self, conn=None, cur=None, idx = None):
        ...
        query = "SELECT CATEGORY, TITLE, CONTENTS, ATCH_IMG_LOCAL_PATH, ATCH_FILE_LOCAL_PATH, ATCH_IMG_PATH, ATCH_FILE_PATH, ATCH_FILE_NAME FROM FORUM WHERE IDX = %s"
    ...
    def save_file_at_dir(self, dst_dir_path, dst_filename, sot_dir_path, sot_filename, mode='wb'):
        # 디렉토리 생성 (존재하지 않는 경우)
        os.makedirs(dst_dir_path, exist_ok=True)

        # 원본 파일 경로
        source_file_path = os.path.join(sot_dir_path, sot_filename)
    ...
    def submit(self, conn, cur, idx, data):
        ...
        query = "UPDATE FORUM SET CATEGORY = %s, TITLE = %s, CONTENTS = %s, ATCH_IMG_PATH = %s, ATCH_FILE_PATH = %s, ATCH_FILE_NAME = %s, ATCH_IMG_LOCAL_PATH = %s, ATCH_FILE_LOCAL_PATH = %s WHERE IDX = %s;"
        cur.execute(query, (category, title, contents, imgs_path, files_path, self.atch_files, local_imgs_path, '', idx))  #local_files_path 필요없어 공백 처리
        conn.commit()
